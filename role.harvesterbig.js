var roleHarvesterbig = {
    run:function(creep){
        if(creep.store[RESOURCE_ENERGY] == 0 && creep.memory.harbig) {
            creep.memory.harbig = false;
        }
        if(creep.store.getFreeCapacity() == 0 && !creep.memory.harbig) {
            creep.memory.harbig = true;
        }
        if(!creep.memory.harbig) {
